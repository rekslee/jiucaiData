...
def updateFinanceToDB():
    ...
    vti = vti.reset_index()
    vti.rename(columns={"Date": "date", "Open": "open", "High": "high",
                        "Low": "low", "Close": "close", "Volume": "vol", }, inplace=True,)
    vti['VTI'] = vti['close']
    ...
    while finance_list:
        ...
        if old_data is not None:
            ...
            try:
                last_day = dt.datetime.strptime(
                    old_data['date'].values[-1], '%Y-%m-%d')
            except:
                last_day = dt.datetime.strptime(
                    old_data['date'].values[-1], '%Y-%m-%d %H:%M:%S')

            # last_day = old_data['date'].values[-1].astype(dt.datetime)

            # last_day=last_day.to_datetime()
            if last_day < yesterday:
                new_df = download(code=symbol, start=last_day,
                                  end=yesterday, source=data_source)
                last_trad_day = new_df.index[-1].to_pydatetime()
                if last_trad_day > last_day:
                    print('更新金融数据')
                    data = analysis.updateFinanceData(
                        symbol, vti, new_df, old_data)
...
def updateMarketBreadthToDB():
    ...
    temp_df = pd.DataFrame(0, index=datas[0]['date'].tolist(), columns=['temp'])
    ...
    while datas:
        ...
        # temp_df['is_above_s_ma'] = data['is_above_s_ma']
        data = pd.concat([temp_df, data], axis=1)
        data['is_above_s_ma'].fillna(0, inplace=True)
...
def updateStockToDB():
    ...
    while datas:
        ...
        if isinstance(df, str):
            data = download(code=df, start=start, end=end, source="yahoo")
            if data is not None:
                data["code"] = df
                data = analysis.stock_analysis(data, 20, 60, 120)
                sql_data.dataToDB(stocks_db, data, df)
            datas.pop(0)
            continue
        ...
        if last_day < yesterday and last_trad_day != last_day:
            ...
            new_df.rename(columns={"Date": "date", "Open": "open", "High": "high",
                                    "Low": "low", "Close": "close", "Volume": "vol", }, inplace=True,)
            ...
            final_data = pd.concat(
                [df.set_index(["date"]).sort_index(), new_df.set_index(["date"]).sort_index()])
            # 删除重复的index
            ...
            sql_data.dataToDB(stocks_db, final_data, final_data['code'])
        datas.pop(0)
